...
uniform sampler2D uTextureCloudNoise;
...
void main(){
    ...
    vec4 txtNoise1 = texture2D(uTextureCloudNoise, vec2(vUv.x + uTime * 0.0001, vUv.y - uTime * 0.00014));
    vec4 txtNoise2 = texture2D(uTextureCloudNoise, vec2(vUv.x - uTime * 0.00002, vUv.y + uTime * 0.000017 + 0.2));
    ...
    vec4 txtShape = texture2D(uTextureShape, newUv);
